...
class SceneManager
{
    ...
    private helperXY: THREE.GridHelper;
    public visibleMenu(flag: boolean):void
    {
        this.helperXY.visible = flag;
    }

    public visibleBound(flag: boolean): void {
    ...
    public click(e: MouseEvent): void {
        e.preventDefault();
        var mouseVector: THREE.Vector3 = new THREE.Vector3();
        mouseVector.x = 2 * (e.clientX / window.innerWidth) - 1;
        mouseVector.y = 1 - 2 * (e.clientY / window.innerHeight);


        this.raycaster.setFromCamera(mouseVector, this.camera);
        var intersects: THREE.Intersection[] = this.raycaster.intersectObjects(SceneManager.instance().selectPack.getNode());
    ...
    public setAutoRotate(flag: boolean): void
    {
    ...
    private updateMaterialParams(params: any):void
    {
    ...
    public addBox3D(box3D: Box3D): void
    {
        // 전체 보이기 하기 위핼서
