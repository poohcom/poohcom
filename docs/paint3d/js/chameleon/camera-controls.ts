...
module Chameleon {
    ...
    var touchProjectionOnBall = (() => {
        ...
        return (event: TouchEvent,
            ...
            eye: THREE.Vector3): THREE.Vector3 => {
            projLocal.set(
                (event.touches[0].pageX - canvasBox.width * 0.5 - canvasBox.left) / (canvasBox.width * .5),
                (canvasBox.height * 0.5 + canvasBox.top - event.touches[0].pageY) / (canvasBox.height * .5),
                0.0
            );
    ...
    export class CameraControlsBase {
        ...
        _getTouchPositionInCanvas(event: TouchEvent) {
            var pos = touchPositionInCanvas(event, this.canvasBox);
            pos.x /= this.canvasBox.width;
            pos.y /= this.canvasBox.height;
            return pos;
        }

        _getTouchProjectionOnBall(event: TouchEvent) {
